...
#True  -> статик, False -> файнтьюнд
FREEZE_EMBEDDINGS = False

RUN_NAME = "cnn_fasttext_static" if FREEZE_EMBEDDINGS else "cnn_fasttext_finetuned"
...
#%%
#загркзка фасттекст матрицы эмбуддингов
def load_fasttext_embedding_matrix(path: Path):
    if not path.exists():
        raise FileNotFoundError(f"fastText embedding matrix not found at {path}")
...
def main() -> None:
    ...
    print("\n=== FASTTEXT SANITY CHECK ===")
    print("vocab size:", len(vocab))
    print("embedding matrix shape:", tuple(embedding_matrix.shape))
    print("pad row norm:", torch.norm(embedding_matrix[0]).item())
    print("unk row norm:", torch.norm(embedding_matrix[1]).item())

    for token in ["в", "и", "на", "когда", "было", "гномы"]:
        idx = vocab[token]
        print(token, "idx:", idx, "norm:", torch.norm(embedding_matrix[idx]).item())
